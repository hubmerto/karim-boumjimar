...
for (const exhibition of imported) {
  ...
  // ProjectPanel lookup in the app. Trailing credit paragraphs (Text by /
  // Photography by / Curation by / Spatial design ...) are split out so
  // the panel can render them in their own box under the body.
  if (exhibition.description) {
    const key = `${c.label}|${c.year}`;
    descriptions[key] = splitCredits(exhibition.description);
  }
}
...
function splitCredits(text) {
  const paragraphs = text
    ...
    .filter(Boolean);
  const creditRx =
    /^(text by|curation|curated|photography|photo[\s:]|installation photography|spatial design)\b/i;
  const credits = [];
  while (paragraphs.length && creditRx.test(paragraphs[paragraphs.length - 1])) {
    credits.unshift(paragraphs.pop());
  }
  return {
...
const descHeader = `// Long-form per-exhibition descriptions, with trailing credit paragraphs
// (Text by / Photography by / Curation / Spatial design...) split out so
// they can be rendered in their own panel block.
// Keyed by \`\${title}|\${year}\`. Generated by scripts/build-works-v2.mjs.

export type Description = { body: string; credits: string[] };

export const DESCRIPTIONS: Record<string, Description> = `;

const descFooter = `

export function descriptionFor(title: string, year: number | string): Description | undefined {
